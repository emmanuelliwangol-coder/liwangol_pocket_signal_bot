...
# ──────────────────────────────────────────────────
# STARTUP: clear any existing polling sessions
# ──────────────────────────────────────────────────
def clear_telegram_session():
    """
    Delete webhook + do a dummy getUpdates with timeout=0 to
    terminate any existing long-poll held by a previous instance.
    Railway can leave the old container alive for 10-30s, so we
    retry this until Telegram confirms we are the only caller.
    """
    import time
    base = f"https://api.telegram.org/bot{BOT_TOKEN}"

    # 1. Delete webhook (harmless if none active)
    try:
        r = requests.post(f"{base}/deleteWebhook",
                          json={"drop_pending_updates": True}, timeout=10)
        log.info(f"deleteWebhook -> {r.json().get('description','ok')}")
    except Exception as e:
        log.warning(f"deleteWebhook error (non-fatal): {e}")

    # 2. Hammer getUpdates with timeout=0 until we stop getting 409
    #    Each call terminates the OTHER instance's long-poll on Telegram's side.
    for attempt in range(1, 10):
        try:
            r = requests.post(f"{base}/getUpdates",
                              json={"timeout": 0, "limit": 1}, timeout=10)
            data = r.json()
            if data.get("ok"):
                log.info(f"Session clear confirmed (attempt {attempt})")
                break
            desc = data.get("description", "")
            if "Conflict" in desc:
                log.warning(f"Still conflicting (attempt {attempt}) — waiting 5s...")
                time.sleep(5)
            else:
                log.warning(f"getUpdates unexpected: {desc}")
                break
        except Exception as e:
            log.warning(f"getUpdates probe error: {e}")
            time.sleep(3)

    # 3. Final buffer — give Railway's old container time to fully exit
    log.info("Waiting 5s before starting polling...")
    time.sleep(5)
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    ...
    if isinstance(err, Conflict):
        log.critical("Runtime Conflict detected — shutting down for clean Railway restart.")
        import os, signal
...
def main():
    ...
    # Kill any existing Telegram sessions before we start polling
    ...
    app.job_queue.run_repeating(scan_and_send, interval=SCAN_EVERY * 60, first=20)

    log.info(f"SMC PRO Bot online! Scanning every {SCAN_EVERY} min.")
    app.run_polling(
        drop_pending_updates=True,
        allowed_updates=["message"],
        poll_interval=2.0,       # poll every 2s instead of default 0s — gentler on Telegram
