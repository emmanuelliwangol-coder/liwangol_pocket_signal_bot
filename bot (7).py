...
# ──────────────────────────────────────────────────
# ERROR HANDLER — catches Conflict and other errors
# ──────────────────────────────────────────────────
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    from telegram.error import Conflict, NetworkError, TimedOut
    err = context.error
    if isinstance(err, Conflict):
        log.critical("Conflict: another bot instance is running. Exiting so Railway can restart clean.")
        import os
        os.kill(os.getpid(), 15)   # SIGTERM — clean Railway shutdown
...
def main():
    ...
    # Step 1: Delete any active webhook before polling.
    # This prevents 409 Conflict on Railway redeploy.
    try:
        resp = requests.post(
        ...
        log.info(f"deleteWebhook -> {resp.json()}")
    except Exception as e:
        log.warning(f"deleteWebhook failed (non-fatal): {e}")

    # Step 2: Short delay so the old Railway instance finishes shutting down.
    time.sleep(3)
    ...
    # Catches Conflict + other runtime errors gracefully
    app.add_error_handler(error_handler)

    app.job_queue.run_repeating(scan_and_send, interval=SCAN_EVERY * 60, first=15)
